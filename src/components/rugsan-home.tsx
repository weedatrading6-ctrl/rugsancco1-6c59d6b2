...
import { ArrowDown, ArrowRight, Check, Facebook, Mail, MapPin, Menu, Phone, X } from "lucide-react";
...
import logoImage from "@/assets/rugsan-logo.png";
import brandImage from "@/assets/rugsan-verified-architecture.jpg";
import projectImage from "@/assets/rugsan-verified-project.jpg";
...
function BrandMark({ compact = false }: { compact?: boolean }) {
  return <a href="#home" className="brand-mark" aria-label="Rugsan home"><span className="brand-symbol" aria-hidden="true"><i /><i /><i /></span><span><strong>RUGSAN</strong>{!compact && <small>CONSTRUCTION CO.</small>}</span></a>;
}
  ...
  return <div className="site-shell" dir={rtl ? "rtl" : "ltr"}>
    ...
    <main>
      <section id="home" className="hero">
        <img src={projectImage} alt="Rugsan residential architectural design" fetchPriority="high" />
        <div className="hero-overlay" />
        <div className="container hero-content"><p className="eyebrow hero-kicker">{copy.heroEyebrow}</p><h1>{copy.heroTitle}</h1><p className="hero-copy">{copy.heroBody}</p><div className="hero-buttons"><Button variant="hero" size="xl" onClick={() => go("projects")}>{copy.projectsCta}<ArrowRight /></Button><Button variant="heroOutline" size="xl" onClick={() => go("contact")}>{copy.consultCta}</Button></div></div>
      ...
      <section id="projects" className="section projects-section"><div className="container"><Reveal><p className="eyebrow">{copy.projectsLabel}</p><div className="section-heading"><h2>{copy.projectsTitle}</h2></div></Reveal><div className="projects-grid">{[0,1,2].map((item) => <Reveal className={`project-card project-${item + 1}`} key={item}><button type="button" onClick={() => go("contact")} aria-label={copy.projectTitles[item]}><img src={projectImage} alt={copy.projectTitles[item]} loading="lazy" /><span className="project-shade"/><span className="project-caption"><small>{copy.categories[item]}</small><strong>{copy.projectTitles[item]}</strong></span></button></Reveal>)}</div></div></section>

      <section className="section why-section"><div className="container why-grid"><Reveal><p className="eyebrow">{copy.whyLabel}</p><h2>{copy.whyTitle}</h2></Reveal><div className="reasons">{copy.reasons.map((reason, i) => <Reveal className="reason" key={reason[0]}><span>0{i + 1}</span><div><h3>{reason[0]}</h3><p>{reason[1]}</p></div></Reveal>)}</div></div></section>

      <section className="section process-section"><div className="container"><Reveal><p className="eyebrow">{copy.processLabel}</p><h2>{copy.processTitle}</h2></Reveal><div className="process-grid">{copy.process.map((step, i) => <Reveal className="process-step" key={step[0]}><span>0{i + 1}</span><h3>{step[0]}</h3><p>{step[1]}</p></Reveal>)}</div></div></section>

      <section className="mission-section"><img src={projectImage} alt="Rugsan architectural project" loading="lazy" /><div className="mission-overlay"/><Reveal className="container mission-content"><p className="eyebrow">{copy.missionLabel}</p><h2>{copy.missionTitle}</h2><p>{copy.missionBody}</p></Reveal></section>

      <section id="founder" className="section founder-section"><div className="container founder-grid"><Reveal className="founder-placeholder"><span className="founder-monogram">NM</span><p>{copy.portrait}</p></Reveal><Reveal className="founder-copy"><p className="eyebrow">{copy.founderLabel}</p><h2>{copy.founderTitle}</h2><h3>{copy.founderRole}</h3><p>{copy.founderBody}</p></Reveal></div></section>
